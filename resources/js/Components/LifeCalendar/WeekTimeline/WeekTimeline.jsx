...
const DefaultData = {
  events: [{ type: 1, date: '1995-11-13', title: 'My birthday' }]
};
...
const transformData = (data = { events: [] }) => {
  ...
  _data.events = _data.events.map((item, idx) => {
...
export default function WeekTimeline({ data }) {
    ...
    return (
      <>
        {dt.map((_, idx) => {
          ...
          const obj = _data.events.find((d) => d._weekNum === idx);
          if (obj) {
            bgColor = 'gray';
